...
/* callback funkcija koja se poziva od strane libpcap za svaki dolazeci paket */
...
/* funkcija koja proverava da li su svi paketi primljeni */
...
/* funkcije tredova za wi-fi i eth */
...
int main(int argc, char *argv[]) {
	...
	printf("allocated %d bytes of memory for received array\n", packet_no);
	size_t total_size = packet_no * MAX_DATA_SIZE;
	memory = (unsigned char*) malloc(total_size);
	printf("allocated %ld bytes of memory for memory array\n", sizeof(memory));
	...
	fwrite(memory, sizeof(unsigned char), file_size, f);
