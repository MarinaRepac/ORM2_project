...
uint8_t src_eth_mac_address[6] = { 0x2c, 0x4d, 0x54, 0x56, 0x99, 0xe8 };
uint8_t src_eth_ip_address[4] = { 10, 81, 31, 53 };
uint8_t src_wifi_mac_address[6] = { 0x00, 0x0f, 0x60, 0x05, 0xb7, 0x54 };
uint8_t src_wifi_ip_address[4] = { 10, 81, 2, 70 };

uint8_t dst_eth_mac_address[6] = {0x2c, 0x4d, 0x54, 0x56, 0x9a, 0x1f };
uint8_t dst_eth_ip_address[4] = { 10, 81, 31, 70 };
uint8_t dst_wifi_mac_address[6] = { 0x00, 0x0f, 0x60, 0x08, 0x29, 0x23 };
uint8_t dst_wifi_ip_address[4] = { 10, 81, 2, 51 };
...
/* kontrolne promenljive za tredove */
...
/* callback funkcija koja se poziva od strane libpcap za svaki odlazeci paket */
/* param --> stanje sesije za hvatanje paketa */
...
void *wifi_thread_function(void *param) {
	...
	while (next_packet < packet_no) {
		...
		packet *p = create_packet(eh, ih, uh, ch, memory + MAX_DATA_SIZE * pack, data_size);	// memory je buffer u kom se nalazi slika (tj. iscitani biti)
		int retries = 0;
...
/* obe funkcije salju velicinu paketa sa zaglavljima (samo zavisi da li se koristi eth ili wi-fi) */
